import { useDashboardStats, useStockTrend, useRecentActivity, useTopProducts, useCategoryDistribution } from "../../hooks/useInventory";
...
function KpiCard({ label, value, deltaLabel, positive, accentColor, icon }: KpiCardProps) {
  return (
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="rounded-[16px] border border-[#2a2f42] bg-[#13161e] p-5">
          <div className="mb-4 flex items-center justify-between">
            ...
            <span className="rounded-full bg-[rgba(74,222,128,0.12)] px-2.5 py-0.5 text-[11px] font-semibold text-[#4ade80]">
              4 zones
            </span>
          </div>
          <div className="flex items-center gap-4">
            ...
            <div className="flex flex-1 flex-col">
              {(categories ?? []).map((cat, i) => (
                <div key={cat.category} className="flex items-center gap-3 border-b border-[#2a2f42] py-2 last:border-b-0">
                  <span className="flex-1 text-[13px] text-[#e8eaf0]">{cat.category}</span>
                  <div className="h-1.5 w-20 overflow-hidden rounded-full bg-[#232839]">
                    <div
                  ...
                  <span className="w-9 text-right font-['JetBrains_Mono',monospace] text-[12px] text-[#8b92a8]">
                    {cat.percentage}%
                  </span>
                </div>
              ))}
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="rounded-[16px] border border-[#2a2f42] bg-[#13161e] p-5">
          ...
          <div className="flex flex-col">
            ...
            {(activity ?? []).length === 0 && (
              <p className="py-6 text-center text-[13px] text-[#555d73]">No recent activity.</p>
            )}
          </div>
        </div>
        ...
        <div className="rounded-[16px] border border-[#2a2f42] bg-[#13161e] p-5">
          ...
          <div className="flex flex-col">
            {(topProducts ?? []).map((p) => (
              <div key={p.productId} className="flex items-center gap-3 border-b border-[#2a2f42] py-2.5 last:border-b-0">
                <div className="flex-1 overflow-hidden">
                  ...
                  <span className="text-[12px] text-[#8b92a8]">{p.name}</span>
                </div>
                <div className="flex h-9 items-end gap-0.5">
                  {(p.sparkData ?? []).map((v, i) => (
                    <div
                      key={i}
                      className="w-1 min-w-[3px] rounded-t-sm bg-[#4ade80] opacity-70"
                      style={{ height: `${v}%` }}
                    />
                  ))}
                </div>
            ...
            {(topProducts ?? []).length === 0 && (
              <p className="py-6 text-center text-[13px] text-[#555d73]">No data yet.</p>
            )}
          </div>
        </div>
      </div>
